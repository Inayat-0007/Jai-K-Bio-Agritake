...
function BranchesApp() {
  try {
    ...
    return (
      <div className="min-h-screen bg-[var(--bg-primary)]" data-name="branches-app" data-file="branches-app.js">
          ...
          <div className="py-16 lg:py-20 bg-[var(--bg-primary)]">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="text-center mb-12 lg:mb-16 fade-in">
                <h1 className="text-4xl lg:text-5xl font-bold text-[var(--text-primary)] mb-4 lg:mb-6">{getTranslation(language).branches.title}</h1>
                <p className="text-lg lg:text-xl text-[var(--text-secondary)] max-w-3xl mx-auto">
                  {getTranslation(language).branches.subtitle}
                </p>
              </div>
            </div>
          </div>
          
          <BranchesMap language={language} />
        </main>
    ...
    console.error('BranchesApp component error:', error);
    return null;
  }
}

const root = ReactDOM.createRoot(document.getElementById('root'));
root.render(<ErrorBoundary><BranchesApp /></ErrorBoundary>);